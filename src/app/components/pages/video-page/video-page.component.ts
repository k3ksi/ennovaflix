...
export class VideoPageComponent implements OnInit {
  ...
  onSelect(val: number) {
    this.puntate = []
    this.puntate = Array.from({ length: this.stagioni[val - 1].numero_puntate }, (_, i) => i + 1);
  }

  getFilmOrSerieTv(): void {
    this.route.params.subscribe(params => {
      const id = params['slug'];
      const isFilmOrSerieTv: string = window.location.href.split('/').reduce((acc: string, urlsegment: string, index: number, array: string[]) => {
      ...
      this.details = (isFilmOrSerieTv == "film") ? this.videoService.getFilm(id) : this.videoService.getSerieTV(id);
    });
